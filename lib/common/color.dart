import 'package:flutter/material.dart';

const white= Colors.white;
const black= Colors.black;
const transparent= Colors.transparent;
const blue = Colors.blue;
const colorGrey = Color(0xFF999999);
const colorWhiteTransperent = Color(0x33ffffff);

const primaryColor =  Color(0xFF0092b0);
const lightBlueColor =  Color(0xFFC0FFFF);

const colorAmber_900 =  Color(0xFFFF6F00);
const preIconFillColor = Color(0xFFffa000);
const colorBlackAlpha =  Color(0xaa000000);


const grey_50 =  Color(0xFFfafafa);
const grey_100 =  Color(0xFFf5f5f5);
const grey_200 =  Color(0xFFeeeeee);
const grey_300 =  Color(0xFFe0e0e0);
const grey_400 =  Color(0xFFbdbdbd);
const grey_500 =  Color(0xFF9e9e9e);

const bv_primaryColor =  Color(0xFF0092b0);
const bv_primaryLightColor =  Color(0xFF57c3e2);
const bv_primaryDarkColor =  Color(0xFF006480);
const bv_primaryDarkColor2 =  Color(0xFF003240);
const bv_secondaryLightColor3 =  Color(0xFFd0ebf0);


//ScaffoldMessenger Color

const Color sfDangColor = Colors.redAccent;
const Color sfWarColor = Colors.yellowAccent;
const Color sfDefaultColor = Colors.blueGrey;
const Color sfSucColor = Colors.greenAccent;
const Color sfInfoColor = Colors.lightBlue;
