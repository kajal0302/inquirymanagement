...
/// Stores selected batch Items
List<String> selectedBatchIds = [];

/// Stores selected batch ids
List<Map<String, dynamic>> courseItemsWithFee = [];

/// Stores fetched course list
List<Map<String, dynamic>> selectedCourses = [];

/// Stores selected courses with fees
List<String> courseIds = [];

/// To store selected course IDs
...
class _CourseDetailsState extends State<CourseDetails> {
  ...
  void onBatchSelected(List<String> selectedItems) {
    ...
    selectedBatchIds = selectedItems
        .map((name) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        setState(() {
          ...
          courseIds =
              selectedCourses.map((course) => course["id"].toString()).toList();

          print("================");
          print(courseIds);

          widget.courseController.text =
              selectedCourses.map((e) => e["value"]).join(", ");
  ...
  Future<void> loadstudentCourseListData() async {
    ...
    setState(() {
      if (fetchedCourseListData != null &&
        ...
        courseItemsWithFee = fetchedCourseListData.courses!
            .map((item) => {
                  "id": item.id.toString(),
                  "value": item.name ?? '',
            ...
            .toList();
        courseItems = fetchedCourseListData.courses!
            .map((item) => item.name ?? '')
            .toList();
      } else {
        courseIds = [];
  ...
  Widget build(BuildContext context) {
    ...
    return Form(
      ...
      /// This ensures validation happens after interaction
      child: Column(
        children: [
          ...
          DropDown(
            preSelectedValue: (categoryProvider.category != null &&
                    ...
                    categoryProvider.category!.categories!.any((b) =>
                        b.id.toString() == widget.categoryController.text))
          ...
          CustomCheckBoxTextField(
            ...
            selectedItems:
                selectedCourses.map((e) => e["value"].toString()).toList(),
            onChanged: (selectedItems) {
